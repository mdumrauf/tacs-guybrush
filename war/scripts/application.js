...
// Action Functions
	function loadFeeds(){
		...
		$.each(json, function(key, feeds){
			...
			$newFeed.find('header .feedName').text(feed.name);
			$newFeed.find('header .feedDescription').attr('href', '/getFeedRss?key=' + feed.key);
			...
			$newFeed.insertAfter($formNewFeed);
		});
	}
