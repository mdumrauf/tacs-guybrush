...
release_app_id=385248044922264
...
mvn clean test &&
git checkout . &&
find src/ -type f -exec sed -i "s/$dev_url/$release_url/g" {} + &&
find src/ -type f -exec sed -i "s/$dev_app_id/$release_app_id/g" {} + &&
git add src/ &&
git commit -m "prepare app for release $version_release" &&
mvn --batch-mode release:prepare &&
mvn release:clean &&
find src/ -type f -exec sed -i "s/$release_url/$dev_url/g" {} + &&
find src/ -type f -exec sed -i "s/$release_app_id/$dev_app_id/g" {} + &&
git add src/ &&
